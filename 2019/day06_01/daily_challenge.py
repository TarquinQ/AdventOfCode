...
def count_orbits(orbit_tree_lookup: Dict[str, Node], root_name: str = 'COM'):
    ...
    return count_orbits_inner(root_node, 0, 0)


def count_orbits_inner(curr_orbit: Node, depth: int, curr_count: int) -> int:
    # TODO: Perform Depth-First Count here
    for child in curr_orbit.children:
        curr_count += count_orbits_inner(child, depth + 1, curr_count)
    return curr_count
